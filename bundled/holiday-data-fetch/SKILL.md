...
1. **【硬性·第一步】历史数据询问（F0）**：**无论用户是否主动提及，第一轮都必须明确询问**是否有过往采集/归集的节日数据。若因用户未主动提供就默认为"没有历史数据"而跳过此问，即属违规（历史数据导入是 v1.2 防"越采越少"的核心机制，必须给用户显式的提供机会）。建议话术：
   > "是否有过往采集/归集的节日数据（旧版 `holiday-data-fetch.json`、或历史采集目录）？如有请提供路径，我会先导入为存量基线再采集，避免越采越少；没有的话我直接开始采集。"
...
5. 需要**导入历史采集数据叠加整合**（v1.2 新增；v1.3 起仅 JSON：v1.0 / v1.1 / 现行 `holiday-data-fetch.json` 导入为存量基线）。

若用户要求"直接出完整报告交付物"，应使用 `holiday-data-report`；本技能可为其前置数据采集。
...
| [历史数据路径] | 无 | **硬性必问**：第一轮必须询问用户是否有过往采集数据（旧版 `holiday-data-fetch.json` / 历史采集目录）。有则导入叠加为存量基线；无则在回复注明"本次无历史基线叠加"。详见「交互检查点」 |
...
### F5 交付
- 产物：`holiday-data-fetch.json` + `snapshots/` + `采集日志.csv`（含观测批次/同源复用列）。**无其他产物**——v1.3 起不再派生 `消费数据集.csv` / `现象素材库.json` 等视图。
...
| `scripts/import_history.py` | **历史数据导入器（JSON 识别、质量过滤、归一化、分层判定、导入报告）** | F0 历史导入 |
| `scripts/_merge_core.py` | **分层同一性判定核心（L1–L5 + 观测关联回填），被合并/导入复用** | F3（内部依赖） |
| `scripts/merge_rounds.py` | 增量归集合并去重（分层判定） | F3 |
| `scripts/validate_fetch.py` | 门禁自检（行数分档/维度覆盖/地域粒度/丰富度评分/观测统计/枚举白名单） | F4/F5 |
| `scripts/gen_collection_log.py` | 从 JSON 自动生成标准格式采集日志（含观测批次/同源复用列） | F5 |
| `assets/sample-fetch.json` | 统一 JSON 结构示例 | 对照参考 |

## 兼容性说明

- 本技能产物可直接供 `holiday-data-report` 使用：`layer=L1` → 22 字段数据集；`layer=L2` → 现象素材；`数据性质=预计` 行 → 预测台账；`观测关联` 对 → 预期差分析。**均为 SSOT `items[]` 内的过滤视图，不落成独立文件。**
- **历史数据版本兼容**（v1.3）：v1.0 / v1.1 / 现行 `holiday-data-fetch.json` 均可由 `import_history.py` 消费，导入后不破坏现有 22 字段结构。
- **SSOT 单一出口（v1.3 硬性）**：不再产出 `消费数据集.csv`、不再产出 `现象素材库.json`；`validate_fetch.py` 只做门禁自检、零导出。需要 L1/L2 子集时按 `layer` 直接读 `holiday-data-fetch.json`。

*技能版本：v1.3 | 更新日期：2026-09-18 | v1.3 修订：**SSOT 单一出口——取消 `消费数据集.csv` 与 `现象素材库.json` 两个派生视图**：`validate_fetch.py` 移除 `--export-csv` / `--export-l2` 与 `export_csv()` / `export_l2()`，改为纯门禁自检（结论前打印数据出口声明）；`import_history.py` 移除 CSV 输入分支（`csv_to_items()`、`detect_format` 的 `csv_l1`、目录扫描 `.csv`）及其专属归一化工具（`_norm_caliber` / `CALIBER_ALIAS` / `_norm_value_type` / `_norm_granularity` / `_norm_nature` + 相关常量），历史基线一律来自 `holiday-data-fetch.json`；F5 交付产物收敛为 `holiday-data-fetch.json` + `snapshots/` + `采集日志.csv`。**F0 历史数据询问升级为硬性交互检查点（新增「交互检查点」章节：第一轮必须主动询问、禁止臆断无历史数据、三种答复分支与话术，description 同步携带交互要求）**；新增历史数据叠加整合机制（`import_history.py` 导入存量基线，防"越采越少"）、预测/实测观测序列化管理（`采集时间`/`观测批次`/`观测关联` 三字段，预测实测并存供预期差分析）、数据同一性分层判定（L1–L5，`_merge_core.py` 核心 + `merge_rounds.py`/`import_history.py` 复用）、门禁新增本轮新增占比/预测实测对照/观测重复率/枚举白名单、快照覆盖率排除历史导入行、丰富度评分行数仅计本轮、推算/预计等级口径澄清（权威官方预计可 A/B/C）、采集日志新增观测批次/同源复用列、枚举统一（主题 `支付交易`、地域粒度 `景区商圈级`）。基线：v1.1（6 大缺口维度矩阵、推算 A/B 分类、维度覆盖门禁、基础/丰富双线、地域粒度字段、快照 bug 修复、采集日志自动生成）。*
