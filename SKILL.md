...
**（勾稽基准）全国行**（`地域粒度=全国`）来自全国模式，城市层**引用不重造**，也不另建独立数据文件。
...
- **数据集** → **`holiday-data-fetch.json`（SSOT）**，单一数据源；派生视图不单独交付，需要时现算
...
### 模板与排版
- `references/templates.md` -- 采集日志、22 字段 schema、真实性说明、首页结构、预测台账模板、纵向六子节模板、跨年专题模板
...
### 随包技能（地域=城市时启动）
- `bundled/holiday-data-report-city/` -- **城市维度技能**：`SKILL.md` + 5 份 city references（口径字典/数据源矩阵/模板/可视化规范/开发提示词）+ 3 份 city 模板（报告/首页/图表片段）。地域=城市时启动，通用层引用本技能、不复制；勾稽基准为本技能产出的全国 SSOT
